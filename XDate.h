...
public:
	...
	operator CXStringA() const throw()
	{
	...
	DATE operator=(const CXStringA& date) throw()
	{
		return operator=((LPCSTR)date);
	}

	DATE operator=(const CXString& date) throw()
	{
	...
	bool operator==(CXDate& d) const throw()
	{
		return m_d == d.m_d;
	}
