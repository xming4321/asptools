...
public:
	...
	operator const CXString& () const throw()
	{
		return m_strPath;
	}
	...
	CXPath& operator=( PCWSTR pszPath )
	{
		Clear();
		Append( pszPath );
	...
	CXPath& operator=( const CXPath& path )
	{
		m_bUrl = path.m_bUrl;
		m_strPath = path.m_strPath;
		return *this;
	...
	CXPath& operator+=( PCWSTR pszMore )
	{
		Append( pszMore );
