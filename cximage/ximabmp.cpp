...
bool CxImageBMP::Decode(CxFile * hFile)
{
  ...
  try {
	...
	if(head.biClrUsed > bmpHeader.biClrUsed)
		head.biClrUsed = bmpHeader.biClrUsed;
    ...
    if (pRgb){
		if(head.biClrUsed)
			if (bIsOldBmp){
				...
				hFile->Read((void*)pRgb,head.biClrUsed * sizeof(RGBTRIPLE),1);
				for (int i=head.biClrUsed-1; i>=0; i--){
				...
				hFile->Read((void*)pRgb,head.biClrUsed * sizeof(RGBQUAD),1);
