...
void CxImage::SetTransColor(RGBQUAD rgb)
{
	...
	{
		info.nBkgndIndex = GetNearestIndex(rgb);

		if(info.nBkgndIndex >= 0)
		{
			RGBQUAD* ppal=GetPalette();
			info.nBkgndColor = ppal[info.nBkgndIndex];
			info.nBkgndColor.rgbReserved=0;
		}else info.nBkgndColor.rgbBlue = info.nBkgndColor.rgbGreen = info.nBkgndColor.rgbRed = info.nBkgndColor.rgbReserved = 255;
	}
	else
	{
		info.nBkgndIndex = 0;
