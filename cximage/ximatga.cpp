...
bool CxImageTGA::Decode(CxFile *hFile)
{
  ...
  try
  {
	...
	head.biClrUsed = tgaHead.CmapLength;
...
bool CxImageTGA::Encode(CxFile * hFile)
{
    ...
    tgaHead.CmapLength=(head.biBitCount == 8) ? head.biClrUsed : 0;   // Color Map Length
	...
	if (head.biBitCount==8){
		...
		for (int i=0;i<head.biClrUsed; i++){
			pal[i].r = ppal[i].rgbBlue;
			pal[i].g = ppal[i].rgbGreen;
			pal[i].b = ppal[i].rgbRed;
		}
		hFile->Write(&pal,head.biClrUsed*sizeof(rgb_color),1);
	}
