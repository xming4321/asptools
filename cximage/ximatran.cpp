...
bool CxImage::DecreaseBpp(DWORD nbit, bool errordiffusion, RGBQUAD* ppal, DWORD clrimportant)
{
	...
	case 4:
		...
		{
			CQuantizer q(16, 4);
			q.ProcessImage(GetDIB(), info.pImage);
			tmp.head.biClrUsed = q.GetColorCount();
			ppal=(RGBQUAD*)calloc(q.GetColorCount()*sizeof(RGBQUAD),1);
			q.SetColorTable(ppal);
			tmp.SetPalette(ppal,q.GetColorCount());
			free(ppal);
		}
		break;
	case 8:
	...
	if (info.nBkgndIndex!=-1)
		tmp.SetTransColor(info.nBkgndColor);
...
bool CxImage::QIShrink(long newx, long newy, CxImage* const iDst)
{
    ...
    if (!IsIndexed()) {
		...
		for(int y=0; y<oldy; y++){                                    //for all source rows
			info.nProgress = (long)(100*y/oldy);
			ey += newy;                                                   
